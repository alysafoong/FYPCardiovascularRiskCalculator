...
with st.form("input_form"):
    ...
    PhysicalHealthDays = st.slider("Poor Physical Health (past 30 days)", 0, 30, 0)
    MentalHealthDays = st.slider("Poor Mental Health (past 30 days)", 0, 30, 0)
    LastCheckupTime = st.selectbox("Last Checkup Time", list(last_checkup_map.keys()))
    PhysicalActivities = st.selectbox("Physical Activities", list(yes_no_map.keys()))
    SleepHours = st.slider("Sleep Hours", 0, 24, 7)
    RemovedTeeth = st.selectbox("Number of Teeth Removed", list(removed_teeth_map.keys()))
    HadAngina = st.selectbox("Had Angina", list(yes_no_map.keys()))
    HadStroke = st.selectbox("Had Stroke", list(yes_no_map.keys()))
    HadAsthma = st.selectbox("Had Asthma", list(yes_no_map.keys()))
    HadSkinCancer = st.selectbox("Had Skin Cancer", list(yes_no_map.keys()))
    HadCOPD = st.selectbox("Had COPD", list(yes_no_map.keys()))
    HadDepressiveDisorder = st.selectbox("Had Depressive Disorder", list(yes_no_map.keys()))
    HadKidneyDisease = st.selectbox("Had Kidney Disease", list(yes_no_map.keys()))
    HadArthritis = st.selectbox("Had Arthritis", list(yes_no_map.keys()))
    HadDiabetes = st.selectbox("Had Diabetes", list(diabetes_map.keys()))
    DeafOrHardOfHearing = st.selectbox("Deaf or Hard of Hearing", list(yes_no_map.keys()))
    DifficultyConcentrating = st.selectbox("Difficulty Concentrating", list(yes_no_map.keys()))
    DifficultyWalking = st.selectbox("Difficulty Walking", list(yes_no_map.keys()))
    DifficultyDressingBathing = st.selectbox("Difficulty Dressing/Bathing", list(yes_no_map.keys()))
    ...
    AlcoholDrinkers = st.selectbox("Alcohol Drinkers", list(yes_no_map.keys()))
