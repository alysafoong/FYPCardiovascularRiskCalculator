...
with st.form("input_form"):
    ...
    LastCheckupTime = st.selectbox("Last Checkup Time", [0, 1, 2, 3])
    PhysicalActivities = st.selectbox("Physical Activities", [0, 1])
    SleepHours = st.slider("Sleep Hours", 0, 24, 7)
    RemovedTeeth = st.selectbox("Number of Teeth Removed", [0, 1, 2, 3])
    ...
    HadDiabetes = st.selectbox("Had Diabetes", [0, 1, 2, 3])
    DeafOrHardOfHearing = st.selectbox("Deaf or Hard of Hearing", [0, 1])
    DifficultyConcentrating = st.selectbox("Difficulty Concentrating", [0, 1])
    ...
    SmokerStatus = st.selectbox("Smoker Status", [0, 1, 2, 3])
    ChestScan = st.selectbox("Chest Scan", [0, 1])
    RaceEthnicityCategory = st.selectbox("Race/Ethnicity", [0, 1, 2, 3, 4, 5, 6])
    AgeCategory = st.selectbox("Age Category", list(range(0, 13)))
