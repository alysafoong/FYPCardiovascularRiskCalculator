...
with st.form("input_form"):
    ...
    GeneralHealth = st.slider("General Health", 0, 4, 2)
    ...
    PhysicalHealthDays = st.slider("Physical Health", 0, 30, 0)
    ...
    MentalHealthDays = st.slider("Mental Health", 0, 30, 0)
    
    st.markdown("_About how long has it been since you last visited a doctor for a routine checkup?_")
    LastCheckupTime = st.selectbox("Last Medical Checkup Time", list(last_checkup_map.keys()))
    ...
    PhysicalActivities = st.selectbox("Physical Activities", list(yes_no_map.keys()))
    ...
    RemovedTeeth = st.selectbox("Number of Permanent Teeth Removed", list(removed_teeth_map.keys()))
    
    st.markdown("_Ever told you had angina or coronary heart disease?_")
    HadAngina = st.selectbox("Angina or Coronary Heart Disease", list(yes_no_map.keys()))
    
    st.markdown("_Ever told you had a stroke?_")
    HadStroke = st.selectbox("Stroke", list(yes_no_map.keys()))
    
    st.markdown("_Ever told you had asthma?_")
    HadAsthma = st.selectbox("Asthma", list(yes_no_map.keys()))
    
    st.markdown("_Ever told you had skin cancer that is not melanoma?_")
    HadSkinCancer = st.selectbox("Skin Cancer", list(yes_no_map.keys()))
    
    st.markdown("_Ever told you had COPD (chronic obstructive pulmonary disease), emphysema or chronic bronchitis?_")
    HadCOPD = st.selectbox("COPD", list(yes_no_map.keys()))
    ...
    DifficultyConcentrating = st.selectbox("Difficulty Concentrating or Remembering", list(yes_no_map.keys()))
    
    st.markdown("_Do you have serious difficulty walking or climbing stairs?_")
    DifficultyWalking = st.selectbox("Difficulty Walking or Climbing Stairs", list(yes_no_map.keys()))
    
    st.markdown("_Do you have difficulty dressing or bathing?_")
    DifficultyDressingBathing = st.selectbox("Difficulty Dressing or Bathing", list(yes_no_map.keys()))
    ...
    ChestScan = st.selectbox("Chest Scan", list(yes_no_map.keys()))
    ...
    AlcoholDrinkers = st.selectbox("Alcohol Drinking", list(yes_no_map.keys()))
...
if submitted:
    ...
    st.write("", "High Cardiovascular Risk" if prediction == 1 else "Low Cardiovascular Risk")
